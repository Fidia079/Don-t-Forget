...
void TaskLED(void *pvParameters)  // This is a task.
{
   ...
   for (;;) // A Task shall never return or exit.
  {
    ...
    if(dist_flag == 1) digitalWrite(LED_Dist, LOW);
    else digitalWrite(LED_Dist, HIGH);

    if(light_flag == 1) digitalWrite(LED_Light, LOW);
    else digitalWrite(LED_Light, HIGH);

    if(RFID_flag == 0) digitalWrite(LED_RFID, LOW);
    else digitalWrite(LED_RFID, HIGH);
